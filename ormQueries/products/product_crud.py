...
class ProductCrud:
    ...
    def products_by_manufacturer_with_name_like(cls, name_like):
        return Product.objects.filter(manufacturer__contains=name_like)

    @classmethod
    def manufacturer_names_for_query(cls, query):
        return Product.objects.filter(manufacturer__contains=query).values_list('manufacturer', flat=True)
